...
class Graf:
    ...
    def dist_euristica(self, elem, i_elem, j_elem, reprezentare):
        ind = reprezentare.index(elem)
        i_scop = ind // Nod.NR_COLOANE
        j_scop = ind % Nod.NR_COLOANE
        return ((i_elem - i_scop) ** 2) + ((j_elem - j_scop) ** 2)
...
def get_lista_solutii(l):
    lista_de_mutari = []
    for x in l:
        lista_de_mutari.append(x.nod_graf.info)
    return lista_de_mutari
            ...
            if (not nod_curent.contine_in_drum(nod)):
                ...
                if x is not None:
                    ...
                    if x is not None:
                        ...
                        # def __init__(self, nod_graf, succesori=[], parinte=None, g=0, f=None):
                        nod_nou_de_cautare = NodCautare(nod, parinte=nod_curent, g=g_succesor)
                        open.append(nod_nou_de_cautare)
